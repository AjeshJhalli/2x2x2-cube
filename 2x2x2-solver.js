...
function optimalSolve () {
	...
	let states = [solvedCube];
	
	while (states.length < 3674160) {
		let state = states.shift();
	
		let prevMove = state.moves;
		
		if (prevMove & 0xF != 0)
			while (prevMove & 0xF0 != 0) 
				prevMove >>= 4;
		
		for (let i = 0; i < moves.length; i++) {
			let next = turnCube(state.front, state.up, state.right,
								state.back, state.bottom, state.left,
								state.moves, state.movesPtr, prevMove, moves[i]);

			if (!contains(states, next))
				states.push(next);
		}
	}
	...
	fs.writeFile("table2.json", JSON.stringify(states), function(err) {
		if(err) {
...
function contains(states, cube) {

	states.forEach(state => {
		if (cube.front == state.front &&
			cube.up == state.up &&
			cube.right == state.right &&
			cube.back == state.back &&
			cube.bottom == state.bottom &&
			cube.left == state.left)
			return true;
	});
